...
export function Reviews() {
  return (
    <div className="py-10 border-b border-(--color-border-light)" id="reviews">
      <div className="text-center">
        ...
        <div className="flex items-center justify-center">
          <LaurelLeaf className="h-27.5 w-auto shrink-0 scale-x-[-1]" />
          <span className="text-[100px] font-medium tracking-[-0.03em] leading-none mx-1">
            {listing.rating}
          </span>
          <LaurelLeaf className="h-27.5 w-auto shrink-0" />
        </div>
      ...
      <div className="grid grid-cols-7 mt-10">
          ...
          <div className="space-y-1">
            {listing.ratingDistribution.map((row) => (
              <div key={row.stars} className="flex items-center gap-3 text-xs">
                <span className="w-2 text-(--color-text-secondary)">
                  {row.stars}
                </span>
                <div className="flex-1 h-1 rounded-full bg-(--color-border-light) overflow-hidden">
                  <div
        ...
        {listing.reviewBreakdown.map((item) => (
          <div
            key={item.label}
            className="px-6 border-l border-(--color-border)"
          >
            <p className="text-sm font-medium mb-3">{item.label}</p>
            <Icon name={item.icon} size={32} className="text-(--color-text) mb-2" />
            <p className="text-lg font-medium">{item.value}</p>
          </div>
        ))}
      </div>

      <div className="flex gap-3 overflow-x-auto mt-8 pb-1 scrollbar-none [&::-webkit-scrollbar]:hidden">
        {reviewTags.map((tag) => (
          <button
            key={tag.label}
            type="button"
            className="shrink-0 flex items-center gap-1.5 border border-(--color-border) rounded-full px-3.5 py-2 text-sm hover:border-(--color-text) transition-colors"
          >
            {tag.label} <span className="text-(--color-text-secondary)">{tag.count}</span>
          </button>
        ))}
      </div>
          ...
          <div key={review.id}>
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-(--color-text-secondary)">{review.tenure}</p>
              </div>
            </div>
            <div className="flex items-center gap-1 mt-2 text-xs text-(--color-text-secondary)">
      ...
      <button
        type="button"
        className="mt-8 border border-(--color-text) rounded-lg px-5 py-3 text-sm font-medium hover:bg-(--color-bg-subtle)"
      >
