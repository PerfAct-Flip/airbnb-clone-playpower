...
export function ThingsToKnow() {
  return (
    <div className="py-10 border-b border-[var(--color-border-light)]">
      <h2 className="text-[26px] leading-[30px] font-medium mb-6">Things to know</h2>
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        {COLUMNS.map((col) => (
          <div key={col.title}>
            ...
            {col.lines.map((line) => (
              <p key={line} className="text-sm text-[var(--color-text-secondary)] mb-1">
                {line}
              </p>
            ))}
